...
{
    putDirect(exec->propertyNames().prototype, JSSVGFilterElementPrototype::self(exec, globalObject), DontDelete | ReadOnly);
}
...
{
    return new (exec) JSSVGFilterElementPrototype(globalObject, JSSVGFilterElementPrototype::createStructure(JSSVGElementPrototype::self(exec, globalObject)));
}
...
EncodedJSValue JSC_HOST_CALL jsSVGFilterElementPrototypeFunctionSetFilterRes(ExecState* exec)
{
    ...
    SVGFilterElement* imp = static_cast<SVGFilterElement*>(castedThis->impl());
    unsigned filterResX = exec->argument(0).toInt32(exec);
    unsigned filterResY = exec->argument(1).toInt32(exec);
