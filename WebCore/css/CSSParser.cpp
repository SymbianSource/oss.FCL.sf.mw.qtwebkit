...
bool CSSParser::parseValue(int propId, bool important)
{
    ...
    case CSSPropertyWebkitHapticTapStrength:
        if (id == CSSValueNone || id == CSSValueWeak || id == CSSValueMedium || id == CSSValueStrong)
            validPrimitive = true;
        break;
    case CSSPropertyWebkitHapticTap: {
        const int properties[2] = { CSSPropertyWebkitHapticTapType, CSSPropertyWebkitHapticTapStrength};
        return parseShorthand(propId, properties, 2, important);
    }
#endif
#if ENABLE(WCSS)
